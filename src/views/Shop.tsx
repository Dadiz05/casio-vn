...
import { requestAiRecommendation, toProductBrief } from '@/lib/ai'
...
export default function Shop() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (movement !== 'all') {
      result = result.filter((product) => product.movement === movement)
    }
    ...
    else if (sortBy === 'popular') result.sort((a, b) => b.sold - a.sold)
  ...
  const aiSuggestedProducts = useMemo(() => {
    if (!aiResult) return []

    const productById = new Map(products.map((product) => [product.id, product]))
    return aiResult.suggestedProductIds.flatMap((productId) => {
      const product = productById.get(productId)
      return product ? [product] : []
    })
  }, [aiResult, products])

  const aiSuggestionByProductId = useMemo(() => {
    const suggestionMap = new Map<string, string>()
    aiResult?.suggestions.forEach((suggestion) => {
      suggestionMap.set(suggestion.productId, suggestion.reason)
    })
  ...
  const handleFilterChange =
    (setter: Dispatch<SetStateAction<string>>) => (event: ChangeEvent<HTMLSelectElement>) => {
      setter(event.target.value)
      setCurrentPage(1)
    }

  const handleAiRecommendation = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault()

    const query = aiQuery.trim()
    if (!query) {
      setAiError('Nhập nhu cầu trước khi tư vấn.')
      return
    }
    ...
    try {
      ...
      setAiError(error instanceof Error ? error.message : 'Không gọi được AI tư vấn.')
    } finally {
      setAiLoading(false)
    }
  }
  ...
  return (
    <div className="casio-container casio-section py-10">
      <div className="mb-8 text-center max-w-2xl mx-auto">
        ...
        <h1 className="site-title text-3xl sm:text-4xl mt-3 mb-3">Cửa Hàng Casio</h1>
        <p className="site-copy">Khám phá bộ sưu tập đồng hồ Casio chính hãng</p>
        {searchQuery.trim() && (
          <p className="mt-3 text-sm text-[var(--color-text-secondary)]">
            Kết quả cho:{' '}
            <span className="font-medium text-[var(--color-text-primary)]">{searchQuery}</span>
          </p>
        )}
      </div>

      <div className="site-card p-5 sm:p-7 mb-8 overflow-hidden">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-2xl">
            ...
            <h2 className="site-title mt-3 text-2xl sm:text-3xl">Chọn đồng hồ theo nhu cầu</h2>
          </div>
          {aiResult && (
            <span className="site-chip w-fit">
              {aiResult.source === 'openai' ? 'OpenAI' : 'Fallback nội bộ'}
            </span>
          )}
        </div>

        <form onSubmit={handleAiRecommendation} className="mt-5 grid gap-3 lg:grid-cols-[1fr_auto]">
          <textarea
            value={aiQuery}
            onChange={(event) => setAiQuery(event.target.value)}
            className="site-textarea min-h-24 resize-none"
            placeholder="Nam dưới 3 triệu, đi làm, dây kim loại, chống nước tốt"
          />
          <button
            type="submit"
            disabled={aiLoading}
            className="site-button site-button--primary min-w-48 self-stretch lg:self-auto"
          >
        ...
        {aiError && <p className="mt-3 text-sm text-[var(--color-surface-raised)]">{aiError}</p>}

        {aiResult && (
          <div className="mt-5 grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
            <div className="rounded-[var(--radius-sm)] border border-[var(--color-border-strong)] bg-[rgba(16,4,4,0.02)] p-4">
              <p className="font-semibold text-[var(--color-text-primary)]">{aiResult.answer}</p>
              <p className="site-copy mt-2 text-sm">{aiResult.querySummary}</p>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {aiResult.followUpQuestions.map((question) => (
                  <button
                    ...
                    className="rounded-full border border-[var(--color-border-strong)] px-3 py-1.5 text-sm text-[var(--color-text-secondary)] transition hover:border-[var(--color-surface-raised)] hover:text-[var(--color-text-primary)]"
                  >
                    {question}
                  </button>
                ))}
      ...
      {aiSuggestedProducts.length > 0 && (
        <section className="mb-10">
          <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <span className="site-kicker">AI shortlist</span>
              <h2 className="site-title mt-2 text-2xl">Sản phẩm được gợi ý</h2>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {aiSuggestedProducts.map((product) => (
              <div key={product.id} className="space-y-3">
                ...
                {aiSuggestionByProductId.get(product.id) && (
                  <p className="rounded-[var(--radius-sm)] border border-[var(--color-border-strong)] bg-white px-4 py-3 text-sm text-[var(--color-text-secondary)]">
                    {aiSuggestionByProductId.get(product.id)}
                  </p>
                )}
              </div>
      ...
      <div className="site-card p-5 sm:p-7 mb-10">
        <div className="flex flex-col sm:flex-wrap sm:items-end gap-5 lg:gap-6">
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-7 flex-1 gap-3 sm:gap-4 w-full">
            <div>
            ...
            <div>
              ...
              <select
                value={movement}
                onChange={handleFilterChange(setMovement)}
                className="site-select text-sm"
              >
                <option value="all">Tất cả</option>
            ...
            <div>
              ...
              <select
                value={stockStatus}
                onChange={handleFilterChange(setStockStatus)}
                className="site-select text-sm"
              >
                <option value="all">Tất cả</option>
      ...
      {currentProducts.length > 0 ? (
        ...
        <div className="site-card text-center py-16 px-6">
          ...
          <button onClick={resetFilters} className="site-button site-button--primary mt-6">
            Xóa bộ lọc
          </button>
        </div>
      )}
