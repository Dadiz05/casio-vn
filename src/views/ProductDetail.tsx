...
import { ArrowLeft, ShoppingCart, ChevronLeft, ChevronRight, Heart, Star } from 'lucide-react'
import { useState, useEffect } from 'react'

export default function ProductDetail() {
  const { id } = useParams()
  const navigate = useNavigate()
  const { products, addToCart, wishlistIds, toggleWishlist } = useStore()

  const product = products.find((p) => p.id === id)
  const [quantity, setQuantity] = useState(1)
  const [currentImageIndex, setCurrentImageIndex] = useState(0)
  const [selectedColor, setSelectedColor] = useState(product?.colorVariants?.[0]?.value || '')
  ...
  if (!product) {
    return (
      <div className="casio-container casio-section py-20 text-center">
        <h2 className="site-title text-3xl sm:text-4xl">Không tìm thấy sản phẩm</h2>
  ...
  const selectedColorLabel =
    colors.find((color) => color.value === selectedColor)?.name || colors[0]?.name || 'Tiêu chuẩn'
    ...
    <div className="casio-container casio-section py-12 page-fade">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 xl:gap-12">
        ...
        <div>
          ...
          <h1 className="site-title text-3xl sm:text-4xl leading-tight mb-4">{product.name}</h1>

          <div className="mb-6">
            <p className="text-sm text-[var(--color-text-secondary)] mb-2">
              Bạn đang xem <span className="font-semibold text-black">{selectedColorLabel}</span>
            </p>
          ...
          <div className="grid grid-cols-2 gap-3 mb-8">
            <div className="rounded-[var(--radius-sm)] border border-[var(--color-border-strong)] bg-white p-4">
              <p className="text-sm text-[var(--color-text-secondary)]">Tồn kho</p>
              <p className="font-semibold">{product.stock} sản phẩm</p>
            </div>
            <div className="rounded-[var(--radius-sm)] border border-[var(--color-border-strong)] bg-white p-4">
              <p className="text-sm text-[var(--color-text-secondary)]">Bảo hành</p>
              <p className="font-semibold">{product.warrantyMonths} tháng</p>
            </div>
          </div>
          ...
          <div className="space-y-6 mb-8 text-sm">
            <div className="space-y-2">
              <p className="font-medium text-[var(--color-success)] flex items-center gap-2">
                <span className="text-lg">✓</span> Sản phẩm 100% chính hãng CASIO Nhật Bản, bảo hành
                chính hãng
              </p>
              <p className="text-[var(--color-error)] font-medium flex items-center gap-2">
                <span className="text-lg">⚠</span> Hoàn tiền gấp 10 lần nếu phát hiện hàng giả, hàng
                nhái
              </p>
            </div>

            <div className="bg-[var(--color-surface-muted)] p-4 rounded-lg space-y-2">
              <p className="font-medium text-[var(--color-text-primary)]">Chấp nhận thanh toán:</p>
            ...
            <div className="bg-[var(--color-surface-muted)] p-4 rounded-lg">
              ...
              <ul className="list-disc list-inside text-[var(--color-text-secondary)] space-y-1">
                <li>Túi, hộp, thẻ bảo hành, bảo hành điện tử, xuất hoá đơn eVAT</li>
            ...
            <div>
              ...
              <ul className="mt-2 space-y-2 text-sm">
                <li className="flex items-start gap-2">
                  <span className="text-green-600 mt-0.5">•</span> Giảm 100.000đ khi nhập mã{' '}
                  <span className="font-mono bg-gray-100 px-1 rounded">EXP001</span> (đơn từ
                  2.000.000đ)
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-green-600 mt-0.5">•</span> Giảm 200.000đ khi nhập mã{' '}
                  <span className="font-mono bg-gray-100 px-1 rounded">EXP002</span> (đơn từ
                  4.000.000đ)
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-green-600 mt-0.5">•</span> Giảm 300.000đ khi nhập mã{' '}
                  <span className="font-mono bg-gray-100 px-1 rounded">EXP003</span> (đơn từ
                  6.000.000đ)
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-green-600 mt-0.5">•</span> Giảm 20% khi In logo / khắc laser
                  logo, tên, họa tiết lên đồng hồ theo yêu cầu
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 mb-8">
            <span className="font-medium text-[var(--color-text-primary)]">Số lượng:</span>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            ...
            <button
              ...
              className="site-button site-button--ghost py-4 sm:py-5"
            >
              <Heart
                size={20}
                className={isWishlisted ? 'fill-[var(--color-surface-raised)]' : ''}
              />
